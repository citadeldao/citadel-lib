...
export class OasisNetwork extends BaseNetwork {
    ...
    async signTransaction(rawTransaction, { privateKey, derivationPath }) {
      const transaction = await tranformTransaction(rawTransaction.transaction || rawTransaction) 
    ...
    static async createWalletByMnemonic({
        ...
        const signer = await oasis.hdkey.HDKey.getAccountSigner(mnemonic, derPathIndex, passphrase)
